...
function populateTeams(players) {
    ...
    players.forEach((player) => {
        if (!uniqueTeam.includes(player.team)) {
            uniqueTeam.push(player.team);
        }
    });

    let selectionE = document.getElementById("team-filter"); 

    uniqueTeam.forEach((team) => {
        let optionE = document.createElement("option");
        optionE.value = team;
        optionE.innerText = team;
        selectionE.appendChild(optionE); 
    });
...
function doFilter(rows, column, word) {
    ...
    for (let i = 0; i < rows.length; i++) {
        ...
        if (txtValue.toLowerCase().indexOf(word.toLowerCase()) > -1) {
